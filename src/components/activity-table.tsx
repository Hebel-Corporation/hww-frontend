...
import { Button, Chip, Pagination, Table, TableBody, TableCell, TableColumn, TableHeader, TableRow } from '@heroui/react';
        ...
        <>
            ...
            >
                <TableHeader>
                    ...
                    <TableColumn key="nbre_bonus_type" className='hidden sm:table-cell'>Nbre des bonus / Type</TableColumn>
                    <TableColumn key="amount">Montant total</TableColumn>
                    <TableColumn key="status">Actions</TableColumn>
                </TableHeader>
                <TableBody isLoading={isLoading} loadingContent={<GlobalLoader />} 
                    emptyContent={
                        <EmptyData description={`Aucun bonus n'a été enregistré ${filterObj?.label}.`} />
                    } 
                >
                    {
                        data?.results?.map((item: any, i:any) => (
                            <TableRow key={item?.grantee__id+i+item?.bonus_type}>
                                ...
                                <TableCell className='hidden sm:table-cell'>
                                    <Chip size='sm' variant='bordered'>{item?.count} {item?.bonus_type}</Chip>
                                </TableCell>
                                <TableCell>
                                    $ {item?.total_bonus}
                                </TableCell>
                                <TableCell className='px-0 sm:px-3'>
                                    <Button size='sm' onPress={() => openDrawer(item)}>Payer</Button>
                                </TableCell>
                            </TableRow>
                        ))
                    }
