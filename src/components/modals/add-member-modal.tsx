...
export default function AddMemberModal({
  ...
  const form = useForm<z.infer<typeof refineMemberFormSchema>>({
    resolver: zodResolver(
      isFirstNode
        ? memberFormSchema.omit({ parrainId: true, sponsorId: true })
        : refineMemberFormSchema
    ),
    defaultValues: {
      parrainId: "",
      sponsorId: "",
  ...
  async function onSubmit(values: z.infer<typeof refineMemberFormSchema>) {
    ...
    if (isFirstNode) {
      const fieldsSchemaToBeValid = memberFormSchema.omit({
        parrainId: true,
        sponsorId: true,
      });
  ...
  return (
    <>
      <Button
        ...
        className="p-2 text-sm sm:text-normal"
      >
        Ajouter un membre
      </Button>
      <Modal
                    ...
                    <div className="flex flex-col flex-1 gap-5">
                      {isFirstNode ? (
                        <div className="w-full flex gap-2 p-2.5 rounded-sm bg-yellow-50 text-yellow-500">
                          <TriangleAlert />
                          <span>
                            Vous êtes au point d&apos;enregistrer votre premier
